...
export class RepresentationService {
     ...
     */
    static async createRepresentation(agentId, clientId, terms, notes, createdBy = agentId) {
     ...
     */
    static async updateRepresentation(representationId, updates, updatedBy) {
        const representationsCollection = await getRepresentationsCollection();
        const eventsCollection = await getRepresentationEventsCollection();
        ...
        for (const key in updates) {
            ...
            const oldValue = currentRepresentation[field];
        ...
        const result = await representationsCollection.updateOne({ _id: representationId }, updatePayload);
     ...
     */
    static async archiveRepresentation(representationId, archivedBy) {
        const representationsCollection = await getRepresentationsCollection();
        const eventsCollection = await getRepresentationEventsCollection();
        const currentRepresentation = await representationsCollection.findOne({ _id: representationId });
        if (!currentRepresentation) {
        ...
        const result = await representationsCollection.updateOne({ _id: representationId }, updates);
        if (result.modifiedCount === 0) {
        ...
        const event = {
            ...
            changes: [
                ...
                {
                    field: 'endDate',
                    oldValue: deepCopy(currentRepresentation.endDate ?? undefined),
                    newValue: deepCopy(now)
                }
            ],
    ...
    static async changeStatus(representationId, newStatus, updatedBy) {
        return this.updateRepresentation(representationId, { status: newStatus }, updatedBy);
    }
    static async updateTerms(representationId, newTerms, updatedBy) {
        if (!newTerms) {
    ...
    static async endRepresentation(representationId, updatedBy) {
        return this.archiveRepresentation(representationId, updatedBy);
    }
    // --- Fetching methods --- 
    static async getRepresentationById(representationId) {
        const representationsCollection = await getRepresentationsCollection();
     ...
     */
    static async getAgentRoster(agentId, includeInactive = false) {
        const representationsCollection = await getRepresentationsCollection();
        ...
        const roster = await Promise.all(representations.map(async (rep) => {
            const client = await usersCollection.findOne({ _id: rep.clientId, role: 'client' });
            return { ...rep, client: client || null };
        }));
        return roster;
    }
    static async getRepresentationsByAgentId(agentId) {
        const representationsCollection = await getRepresentationsCollection();
        // Call find() then await toArray()
        return representationsCollection.find({ agentId });
    }
    static async getRepresentationsByClientId(clientId) {
        const representationsCollection = await getRepresentationsCollection();
    ...
    static async getEventsByRepresentationId(representationId) {
        const eventsCollection = await getRepresentationEventsCollection();
     ...
     */
    static async getConnectedAgents(clientId) {
        const representationsCollection = await getRepresentationsCollection();
        const usersCollection = await getUsersCollection();
        ...
        const agents = await Promise.all(representations.map(async (rep) => {
        ...
        const filteredAgents = agents.filter((agent) => agent !== null);
     ...
     */
    static async getConnectedClients(agentId) {
        const representationsCollection = await getRepresentationsCollection();
        const usersCollection = await getUsersCollection();
        ...
        const clients = await Promise.all(representations.map(async (rep) => {
        ...
        const filteredClients = clients.filter((client) => client !== null);
