...
export class AuthService {
    ...
    static async registerUser(email, password, registrationCode, name) {
        ...
        if (existingUser) {
            ...
            if (!existingUser.passwordHash || !existingUser.passwordSalt) {
                ...
                const now = new Date();
                await collection.updateOne({ _id: existingUser._id }, {
                    passwordHash,
                    passwordSalt: salt,
                    name: {
                        first: firstName,
                ...
                if (userType === 'client' && existingUser._id) {
                    try {
                        ...
                        for (const representation of representations) {
                            await RepresentationService.updateRepresentation(representation._id, { status: 'active' }, clientId);
                        }
                    }
                    catch (error) {
