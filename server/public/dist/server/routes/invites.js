...
router.post("/", requireAuth, async (req, res) => {
    try {
        ...
        if (existingClient) {
            ...
            const newClient = await ClientService.createClient(email, firstName, lastName);
            clientId = newClient._id;
        }
        // Create the representation
        const representation = await RepresentationService.createRepresentation(new ObjectId(agentId), clientId, {
...
router.get("/", requireAuth, async (req, res) => {
    try {
        ...
        const invitations = await Promise.all(representations.map(async (representation) => {
