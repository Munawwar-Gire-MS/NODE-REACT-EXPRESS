...
calendarRouter.get('/events', requireAuth, async (req, res) => {
    try {
        ...
        if (clientId && typeof clientId === 'string') {
            ...
            const hasRepresentation = representations.some(rep => rep.clientId.equals(new ObjectId(clientId)) &&
                rep.status === 'active');
        ...
        const events = await CalendarService.getEventsForCalendar(new ObjectId(userId), typeof clientId === 'string' ? new ObjectId(clientId) : new ObjectId(userId), // Use clientId as ownerId if provided
...
calendarRouter.put('/events', requireAuth, async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
        ...
        const { title, description, date, time, endDate, endTime, visibility, timeZone, type, isMultiDay } = req.body;
        console.log('Received calendar event creation request:', {
            userId,
            body: { ...req.body, agents: req.body.agents?.length }
        });
        ...
        const visibilityObj = {
            type: visibility.type === 'selected_agents' ? 'selected_agents' : 'private',
            agentIds: visibility.type === 'selected_agents' ? visibility.agentIds?.map((id) => new ObjectId(id)) : undefined
        };
        console.log('Visibility object:', visibilityObj);
        const savedEvent = await CalendarService.createEvent(title, description || '', date, time, endDate, endTime, timeZone, new ObjectId(userId), new ObjectId(userId), // For clients, ownerId is the same as userId
        visibilityObj, type || 'other', undefined, isMultiDay || false);
        // Format dates in user's timezone before sending response
...
calendarRouter.patch('/events/:id', requireAuth, async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
        ...
        const updatedEvent = await CalendarService.updateEvent(new ObjectId(eventId), title, description || '', date, time, endDate, endTime, timeZone, new ObjectId(userId), visibilityObj, type || 'other', isMultiDay || false);
        if (!updatedEvent) {
...
calendarRouter.delete('/events/:id', requireAuth, async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
